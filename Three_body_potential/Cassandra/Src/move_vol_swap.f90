...
SUBROUTINE GEMC_NVT_Volume
  ...
  REAL(DP), ALLOCATABLE :: pair_nrg_vdw_old(:,:), pair_nrg_qq_old(:,:), triad_nrg_vdw_old(:,:,:)
  ...
  IF (l_pair_nrg) THEN
	 ...
	 ALLOCATE(triad_nrg_vdw_old(SUM(max_molecules),SUM(max_molecules),SUM(max_molecules)))

     !!$OMP PARALLEL WORKSHARE DEFAULT(SHARED)
     pair_nrg_vdw_old(:,:) = pair_nrg_vdw(:,:)
     pair_nrg_qq_old(:,:) = pair_nrg_qq(:,:)
	 triad_nrg_vdw_old(:,:,:) = triad_nrg_vdw(:,:,:)
   ...
   ELSE
      ...
      ELSE
         ...
         IF (accept) THEN
            ...
            IF (l_pair_nrg) DEALLOCATE(pair_nrg_vdw_old,pair_nrg_qq_old,triad_nrg_vdw_old)
 ...
 CONTAINS
   ...
   SUBROUTINE Reset_Coords
     ...
     IF (l_pair_nrg) THEN
		...
		triad_nrg_vdw(:,:,:) = triad_nrg_vdw_old(:,:,:)
        !!$OMP END PARALLEL WORKSHARE
        
        DEALLOCATE(pair_nrg_vdw_old,pair_nrg_qq_old,triad_nrg_vdw_old)
     END IF
