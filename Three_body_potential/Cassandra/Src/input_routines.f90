...
! determine the type of molecule input and connectivity
  ...
  IF (l_pair_nrg) THEN
	 ...
	 ALLOCATE( triad_nrg_vdw(SUM(max_molecules),SUM(max_molecules),SUM(max_molecules)), Stat = AllocateStatus)
     IF (AllocateStatus /= 0 ) THEN
        write(*,*) 'memmory could not be allocated for triad_nrg_vdw array'
        write(*,*) 'aborting'
        STOP
     END IF
  END IF
